...
VELOCITY = 7.5
MIDPOINT_VELOCITY = 3
midpoint_bias_ratio = 0.75
waypoint_bias_ratio = 0.75
waypoint_distance_power = 5
waypoint_velocity_power = 6
...
# VELOCITY = 6.5
# MIDPOINT_VELOCITY = 3
# midpoint_bias_ratio = 0.9
# waypoint_bias_ratio = 0.9
# waypoint_distance_power = 5
# waypoint_velocity_power = 4
...
# MIDPOINT_DIS = 10
# VELOCITY = 5
# MIDPOINT_VELOCITY = 3.5
# midpoint_bias_ratio = 1.2
# waypoint_bias_ratio = 1
# waypoint_distance_power = 5
# waypoint_velocity_power = 5
...
def waypoint_create(_midpoint):
    ...
    for i in range(0, len(_midpoint_list) - 1):
        ...
        for j in range(1, _waypoint_num):
            ...
            _point = [_midpoint_list[i][0] + _bias[0] * ratio, _midpoint_list[i][1] + _bias[1] * ratio]
            _dis = WAYPOINT_DIS - (WAYPOINT_DIS - MIDPOINT_DIS) * ratio ** waypoint_distance_power
            if ratio <= 0.5:
                ratio_t = 1 - ratio * 2
                ratio_2_t = ratio_t ** waypoint_velocity_power
                _velocity = MIDPOINT_VELOCITY + (VELOCITY - MIDPOINT_VELOCITY) * (1 - ratio_2_t)
                # _waypoint_list.append([midpoint_cal(_point, _midpoint_list[i], WAYPOINT_DIS * waypoint_bias_ratio),
                ...
                #                        WAYPOINT_DIS, VELOCITY])
            # _waypoint_list.append([midpoint_cal(_point, _midpoint_list[i], _dis * waypoint_bias_ratio), _dis, _velocity])
            _waypoint_list.append([midpoint_cal(_point, _midpoint_list[i], _dis * ((waypoint_bias_ratio - 1)*ratio+1)), _dis, _velocity])
        if i == len(_midpoint_list) - 2:
            _waypoint_list.append(
                [midpoint_cal(_midpoint_list[i + 1], _midpoint_list[i], MIDPOINT_DIS * (midpoint_bias_ratio-0.1)),
                 MIDPOINT_DIS,
                 MIDPOINT_VELOCITY])
        else:
...
while cap.isOpened():
        ...
        if frame is not None:
            ...
            if box_found == 1:
                ...
                if midpoint is not None:
                    ...
                    if COM_ENABLE:
                        str = struct.pack("7B", 0xFF, 0x80, 0x80, 0x80, 0x80, 0x0D, 0x0A)
                        com.write(str)
                    # cv2.waitKey(0)
    ...
    end_time = time.time()
    print('all_time:    %.3fms,        %.3fFPS' % ((end_time - start_time) * 1000, 1 / (end_time - last_time)))
    last_time = end_time
