...
def color_tracing(frame, tracing_color):
    ...
    red_img_list = [IMG('red_in_white', color_red, hsv, color_range['color_red_in_white'], kernel),
                    IMG('red_in_black', color_red, hsv, color_range['color_red_in_black'], kernel), ]
    ...
    if (tracing_color == 'red'):
        ...
        for i, img in enumerate(red_img_list[0:1]):
            ...
            for j in range(0, len(cnts)):
                cv2.drawContours(frame, cnts, j, color_red, 1)
    ...
    if ret:
        ...
        if frame is not None:
            ...
            if laser_center_t is not None:
                ...
                cv2.circle(frame, (int(laser_center[0]), int(laser_center[1])), TRACING_DIS, color_red, 1)
                cv2.line(frame, (int(laser_center[0]), int(laser_center[1])), GREEN_LASER_CENTER, color_green, 1)
